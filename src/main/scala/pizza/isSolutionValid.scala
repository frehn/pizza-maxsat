...
object isSolutionValid {
  ...
  private def slicesHaveEnoughIngredients(problem: PizzaProblem, solution: PizzaSolution): Boolean = {
    solution.slices.exists(slice => {
      val count = rectangle((0 until slice.length), (0 until slice.width)).foldLeft(IngredientCount(0, 0)) { case (count, (i, j)) => {
        problem.ingredient(slice.upperLeft.x + i, slice.upperLeft.y + j) match {
          case Tomato() => IngredientCount(count.tomatos + 1, count.mushrooms)
          case Mushroom() => IngredientCount(count.tomatos, count.mushrooms + 1)
        }
      }
      }
